...
namespace WebSocket4Net
{
    ...
    public partial class JsonWebSocket : IDisposable
    {
        ...
        public JsonWebSocket(string uri, string subProtocol, List<KeyValuePair<string, string>> cookies, List<KeyValuePair<string, string>> customHeaderItems, string userAgent, string origin, WebSocketVersion version)
        {
            ...
            SubscribeEvents();
            // m_WebSocket.Proxy.Connect(new EndPoint())

            //m_WebSocket.Proxy.Connect

            // go through proxy for testing
           // var proxy = new HttpConnectProxy(new IPEndPoint(IPAddress.Parse("127.0.0.1"), 8889));
            //m_WebSocket.Proxy = (SuperSocket.ClientEngine.IProxyConnector)proxy;
